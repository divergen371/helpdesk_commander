...
## 2026-02-12 01:50 UTC

### グラフ可視化（PoC）方針の追記

- サーバー固定レイアウト + Sigma.js で描画する方針を明記
- 再起票は強関係、依存は 2本エッジで表現
- 関係強度はホバー時にスコア表示、隣接エッジ/ノードを強調
- 追加依存は当面不要だが、必要ならElixir側のグラフ/レイアウト系ライブラリ導入を検討

---

## 2026-02-12 01:52 UTC

### 障害間関係の表現ルールを追記

- 関係タイプの色分け、方向矢印、強さのホバー表示
- 影響領域クラスタ、タイムライン分離、不確実性表現
- 重要度をノードサイズで表現する方針を追記
...
- リポジトリ全体を対象に、障害説明文から関連候補を探索
- Gitログ/コミット履歴を主軸に、インデックス化で取りこぼしを補完
