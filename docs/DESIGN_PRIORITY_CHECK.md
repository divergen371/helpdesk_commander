...
## トリアージ通知と「全部P1」対策
### ねらい
- 重要なチケットを素早く検知しつつ、**自己申告だけでP1が乱発される状況を防ぐ**。

### 概念の分離（推奨）
- **Urgency（緊急度）**: 依頼者が申告する主観的な緊急度（期限・停止の有無など）。
- **Priority（優先度）**: 運用側のトリアージ結果（Impact × Urgency）。
- 申告は **Urgency** に限定し、Priority は **権限者/システムが確定**する。

### 「全部P1」対策（最低限のルール）
- P1 は **権限者のみ確定**可能にする（一般ユーザーは申告のみ）。
- P1 申告には **根拠必須**（影響範囲・停止有無・期限など）。
- 申告と内容が乖離している場合は **要レビュー** にする。
- **監査ログ**（誰がいつ/何を根拠に変更したか）を記録する。

### 通知条件（例）
- `priority == P1` かつ `confidence >= 0.7`
- SLA 期限が近い（例: 1時間以内）
- Incident 判定が付与された

### 通知の抑制
- 同一チケットの短時間連続通知は **クールダウン**（例: 30分）で抑制。
